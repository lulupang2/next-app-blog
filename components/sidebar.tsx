...
export default function SideToc({ data }: { data: TocItem[] }) {
  const activeItem: string[] = useActiveHeadings();
  ...
  return (
    <div>
      <ul className={classes.base}>
        {data.map((item, index) => {
          const isActive = activeItem.includes(item.href.replace('#', ''));
          const indentClass = `indent--${item.depth}`;

          return (
            <li
              key={index}
              className={`${classes.listItem} ${styles[indentClass]}`}
              onClick={handleClick(item.href)}
            >
              <span
                className={`${classes.link} ${!isActive ? '' : ' font-bold'}`}
              >
